...
pub(super) fn plugin(app: &mut App) {
    ...
    app.add_systems(
        Update,
        (exit_button, play_button).run_if(in_state(GameState::Title)),
    );
}
...
fn setup_title(mut commands: Commands) {
    commands
        ...
        .column(|column| {
            column
                .spawn(TextBundle::from("Asteroids"))
                .style()
            ...
            column
                .column(|column| {
                    column
                        .container((ButtonBundle::default(), PlayButton), |button| {
                            button
                                .spawn(TextBundle::from("Play"))
                                .style()
                    ...
                    column
                        .container((ButtonBundle::default(), ExitButton), |button| {
                            button
                                .spawn(TextBundle::from("Exit"))
                                .style()
                                .font_color(BLACK.into());
                        })
                        .style()
