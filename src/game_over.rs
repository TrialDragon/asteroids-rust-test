...
pub fn plugin(app: &mut App) {
    ...
    app.add_systems(Update, (restart_button, title_button).run_if(in_state(GameState::GameOver)));
}
...
fn setup_game_over(mut commands: Commands) {
    ...
    commands.ui_builder(UiRoot).column(|column| {
        // Title text.
        column.spawn(TextBundle::from("Game Over")).style()
        ...
        column.column(|column| {
            ...
            |button| {
                button.spawn(TextBundle::from("Restart")).style()
            ...
            |button| {
                button.spawn(TextBundle::from("Title")).style()
                    .align_self(AlignSelf::Center)
                    .font_color(BLACK.into());
            }).style()
                .background_color(GRAY_50.into())
        ...
        .entity_commands().insert(StateScoped(GameState::GameOver));
}
